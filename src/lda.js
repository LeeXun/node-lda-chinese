...
//
var process = function(str, numberOfTopics, numberOfTermsPerTopic, languages, alphaValue, betaValue, randomSeed) {
    ...
    str = str.replace(/（/gi, "");
    str = str.replace(/」/gi, "");
    var sentences = str.match( /[^\。.!\?]+[\。.!\?]+/g );

    sentences.map(function(sen,i){
        var words = nodejieba.cutHMM(sen, true);
        documents.push([]);
        words.map(function(word,j){
            ...
            documents[i].push(vocab.indexOf(word))
        })
    })
    // return {
    //     sentences: sentences,
